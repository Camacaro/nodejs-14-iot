...
test.serial('setup', (t) => {
  ...
  t.truthy(
    AgentStub.hasMany.calledWith(MetricStup),
    'Argunment should be the MetricModel'
  )
  t.truthy(MetricStup.belongsTo.called, 'AgentModel.belongsTo was executed')
  t.truthy(
    MetricStup.belongsTo.calledWith(AgentStub),
    'Argunment should be the AgentModel'
  )
})
...
test.serial('Agent#findById', async (t) => {
  ...
  t.true(AgentStub.findById.calledOnce, 'finById should be call one')
  t.true(
    AgentStub.findById.calledWith(id),
    'findById should be called with id'
  )

  t.deepEqual(agent, agentFixtures.byId(id), 'should be same')
})
...
test.serial('Agent#findByUuId', async (t) => {
  ...
  t.true(AgentStub.findOne.calledOnce, 'findOne should be call one')
  t.true(
    AgentStub.findOne.calledWith(uuidArgs),
    'findOne should be called with id'
  )

  t.deepEqual(agent, agentFixtures.byUuid(uuid), 'agent should be same')
})
